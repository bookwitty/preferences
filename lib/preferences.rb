  ...
  module ClassMethods #:nodoc:
    ...
    def build_preference_scope(preferences, inverse = false)
      ...
      preferences.each do |(group, preference), value|
        ...
        statements << if inverse
                        "#{table}.id IS NOT NULL AND #{table}.value " + (value.nil? ? ' IS NOT NULL' : ' != ?') + (!is_default ? " OR #{table}.id IS NULL" : '')
                      else
                        "#{table}.id IS NOT NULL AND #{table}.value " + (value.nil? ? ' IS NULL' : ' = ?') + (is_default ? " OR #{table}.id IS NULL" : '')
                      end
